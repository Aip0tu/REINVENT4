...
from ..dataprocess.data import StandardScaler, MoleculeDataset, preprocess_smiles_columns, get_task_names
from ..nn_utils import NoamLR


def _load_torch_checkpoint(path: str):
    # PyTorch 2.6+ defaults to weights_only=True, but these legacy FLAME
    # checkpoints also serialize argparse.Namespace objects.
...
def save_checkpoint(path: str,
    ...
    :param scaler: A :class:`~flam.data.scaler.StandardScaler` fitted on the data.
    :param features_scaler: A :class:`~flam.data.scaler.StandardScaler` fitted on the features.
    :param atom_descriptor_scaler: A :class:`~flam.data.scaler.StandardScaler` fitted on the atom descriptors.
    :param bond_feature_scaler: A :class:`~flam.data.scaler.StandardScaler` fitted on the bond_fetaures.
    :param args: The :class:`~flam.args.TrainArgs` object containing the arguments the model was trained with.
...
def load_checkpoint(path: str,
    ...
    :return: The loaded :class:`~flam.models.model.MoleculeModel`.
    """
...
def load_scalers(path: str) -> Tuple[StandardScaler, StandardScaler, StandardScaler, StandardScaler]:
    ...
    :return: A tuple with the data :class:`~flam.data.scaler.StandardScaler`
             and features :class:`~flam.data.scaler.StandardScaler`.
    """
    state = _load_torch_checkpoint(path)
...
def load_args(path: str) -> TrainArgs:
    ...
    :param path: Path where model checkpoint is saved.
    :return: The :class:`~flam.args.TrainArgs` object that the model was trained with.
    """
    args = TrainArgs()
    args.from_dict(vars(_load_torch_checkpoint(path)['args']), skip_unsettable=True)
...
def build_optimizer(model: nn.Module, args: TrainArgs) -> Optimizer:
    ...
    :param args: A :class:`~flam.args.TrainArgs` object containing optimizer arguments.
...
def build_lr_scheduler(optimizer: Optimizer, args: TrainArgs, total_epochs: List[int] = None) -> _LRScheduler:
    ...
    :param args: A :class:`~flam.args.TrainArgs` object containing learning rate arguments.
...
def save_smiles_splits(data_path: str,
    ...
    :param train_data: Train :class:`~flam.data.data.MoleculeDataset`.
    :param val_data: Validation :class:`~flam.data.data.MoleculeDataset`.
    :param test_data: Test :class:`~flam.data.data.MoleculeDataset`.
...
def update_prediction_args(predict_args: PredictArgs,
    ...
    :param predict_args: The :class:`~flam.args.PredictArgs` object containing the arguments to use for making predictions.
    :param train_args: The :class:`~flam.args.TrainArgs` object containing the arguments used to train the model previously.
    :param missing_to_defaults: Whether to replace missing training arguments with the current defaults for :class: `~flam.args.TrainArgs`.
