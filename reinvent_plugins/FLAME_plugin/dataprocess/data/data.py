...
def cache_graph() -> bool:
    r"""Returns whether :class:`~flam.features.MolGraph`\ s will be cached."""
    return CACHE_GRAPH


def set_cache_graph(cache_graph: bool) -> None:
    r"""Sets whether :class:`~flam.features.MolGraph`\ s will be cached."""
    global CACHE_GRAPH
    ...
    def __init__(self,
        ...
        if self.features_generator is not None:
            ...
            for fg in self.features_generator:
                # flam/features/features_generators.py
...
class MoleculeDataset(Dataset):
    ...
    def batch_graph(self) -> List[BatchMolGraph]:
        r"""
        Constructs a :class:`~flam.features.BatchMolGraph` with the graph featurization of all the molecules.

        .. note::
           The :class:`~flam.features.BatchMolGraph` is cached in after the first time it is computed
           and is simply accessed upon subsequent calls to :meth:`batch_graph`. This means that if the underlying
           set of :class:`MoleculeDatapoint`\ s changes, then the returned :class:`~flam.features.BatchMolGraph`
           will be incorrect for the underlying data.

        :return: A list of :class:`~flam.features.BatchMolGraph` containing the graph featurization of all the
        ...
        if self._batch_graph is None:
            ...
            # class BatchMolGraph: flam/features/featurization.py
    ...
    def normalize_features(self, scaler: StandardScaler = None, replace_nan_token: int = 0,
        ...
        """
        Normalizes the features of the dataset using a :class:`~flam.data.StandardScaler`.

        The :class:`~flam.data.StandardScaler` subtracts the mean and divides by the standard deviation
        for each feature independently.

        If a :class:`~flam.data.StandardScaler` is provided, it is used to perform the normalization.
        Otherwise, a :class:`~flam.data.StandardScaler` is first fit to the features in this dataset
        and is then used to perform the normalization.

        :param scaler: A fitted :class:`~flam.data.StandardScaler`. If it is provided it is used,
                       otherwise a new :class:`~flam.data.StandardScaler` is first fitted to this
        ...
        :return: A fitted :class:`~flam.data.StandardScaler`. If a :class:`~flam.data.StandardScaler`
                 is provided as a parameter, this is the same :class:`~flam.data.StandardScaler`. Otherwise,
                 this is a new :class:`~flam.data.StandardScaler` that has been fit on this dataset.
        """
    ...
    def normalize_targets(self) -> StandardScaler:  # for regression tasks
        """
        Normalizes the targets of the dataset using a :class:`~flam.data.StandardScaler`.
        ...
        :return: A :class:`~flam.data.StandardScaler` fitted to the targets.
        """
...
def construct_molecule_batch(data: List[MoleculeDatapoint]) -> MoleculeDataset:
    ...
    Additionally, precomputes the :class:`~flam.features.BatchMolGraph` for the constructed
